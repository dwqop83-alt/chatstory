...
// === PATCH 1: Add compress button HTML in input area ===
if (c.includes('<button class="attach-btn"')) {
  const sb = '<button class="attach-btn" onclick="document.getElementById(\'attachInput\').click()">📎</button>';
  const nb = '<button class="attach-btn" onclick="document.getElementById(\'attachInput\').click()">📎</button>\r\n      <button class="compress-btn" id="compressBtn" onclick="compressContext()" title="Lorebook世界观提取">🗜️</button>';
  c = c.split(sb).join(nb);
  ok++; console.log("1. Compress button HTML");
}

// === PATCH 2: Add compress modal HTML before </body> ===
...
const jsFunctions = `async function compressContext(){
  ...
  try{
    ...
    var resp=await fetch(s.apiBaseUrl.replace(/\\/+$/,'')+'/chat/completions',{
      ...
      body:body,signal:_abortController.signal
    });
    ...
    hideProgress(500);toast('❌ '+e.message,'error');
  }
...
function copyCompressed(){
  ...
  for(var k in cats){
    ...
    for(var i=0;i<cards.length;i++){
      var cc2=cards[i];text+='- '+(cc2.name||cc2.event||cc2.rule||('#'+(i+1)));
      for(var f in cc2){if(f!=='name'&&f!=='event'&&f!=='rule'&&cc2[f])text+=' | '+f+': '+cc2[f]}
      text+='\\n';
    }
    text+='\\n';
  }
...
function buildLoreContext(lore){
  ...
  if(lore.characters&&lore.characters.length){
    parts.push('## 角色');
    for(var i=0;i<lore.characters.length;i++){var cc3=lore.characters[i];parts.push('- '+cc3.name+(cc3.role?' ('+cc3.role+')':'')+(cc3.personality?': '+cc3.personality:''));}
  }
  if(lore.settings&&lore.settings.length){
    parts.push('## 场景');
    for(var i=0;i<lore.settings.length;i++){var ss=lore.settings[i];parts.push('- '+ss.name+(ss.type?' ['+ss.type+']':'')+(ss.description?': '+ss.description.substring(0,80):''));}
  }
  if(lore.plotPoints&&lore.plotPoints.length){
    parts.push('## 情节节点');
    for(var i=0;i<lore.plotPoints.length;i++){var pp=lore.plotPoints[i];parts.push('- '+pp.event+(pp.status?' ['+pp.status+']':''));}
  }
  if(lore.worldRules&&lore.worldRules.length){
    parts.push('## 世界观规则');
    for(var i=0;i<lore.worldRules.length;i++){var ww=lore.worldRules[i];parts.push('- '+ww.rule+(ww.category?' ['+ww.category+']':''));}
  }
  return parts.join('\\n');
}
...
function renderLoreSidebar(){
  ...
  for(var ci=0;ci<cats.length;ci++){
    var cat=cats[ci];var cards=lore[cat.key]||[];
    if(!cards.length)continue;total+=cards.length;
...
c = c.split(sendMsgMarker).join(jsFunctions);
ok++; console.log("4. JS functions");

// === PATCH 5: Add lorebook to state init in load() ===
...
c = c.split(oldLoad).join(newLoad);
ok++; console.log("5. State init");

// === PATCH 6: Add lorebook to system prompt ===
...
c = c.split(oldSys).join(newSys);
ok++; console.log("6. System prompt injection");

// === PATCH 7: Add lorebook sidebar section ===
const secMemEnd = '<div class="side-sec" id="secBackup"';
const loreSec = '<div class="side-sec" id="secLore">\r\n        <div class="side-sec-hdr collapsed" onclick="toggleSec(\'secLore\')"><span>📖 世界观 <span class="arr">▼</span></span></div>\r\n        <div class="side-sec-body hidden"><div class="pad" id="loreSideList"><div style="padding:8px;font-size:11px;color:var(--text-secondary);text-align:center">点击 🗜️ 提取世界观</div></div></div>\r\n      </div>\r\n      ' + secMemEnd;
c = c.split(secMemEnd).join(loreSec);
ok++; console.log("7. Sidebar section");

// === PATCH 8: Add renderLoreSidebar to renderAll calls ===
c = c.split('renderMems();renderSidebar();').join('renderLoreSidebar();renderMems();renderSidebar();');
ok++; console.log("8. renderAll hook");
...
if (b === 0) {
  ...
  console.log("Written! Patches:", ok);
} else console.log("BRACE ERROR");