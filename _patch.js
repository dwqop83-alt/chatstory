...
// Helper: safe replace
function rep(old, rep, name) {
  if (!c.includes(old)) { console.log("FAIL:", name); return false; }
...
// ============================================================
// 1. Rewrite compressContext
// ============================================================
// Target: the entire async function compressContext(){...}
// Find it by locating "async function compressContext(){" and the closing "}" before "function closeCompress"
...
if (ccStart > 0 && ccEnd > 0) {
  const oldCC = c.substring(ccStart, ccEnd);
...
`;
  c = c.substring(0, ccStart) + newCC + c.substring(ccEnd);
  patches++;
  console.log("OK: compressContext rewritten");
} else console.log("FAIL: compressContext", ccStart, ccEnd);

// ============================================================
// 2. Replace old helpers
// ============================================================
// Target: "function closeCompress(){" ... "var _compressStats='';"
const hlprStart = c.indexOf("function closeCompress(){");
const hlprEnd = c.indexOf("async function sendMsg(){", hlprStart);
if (hlprStart > 0 && hlprEnd > 0) {
  const oldH = c.substring(hlprStart, hlprEnd);
  const newH = `function closeCompress(){G('compressModal').style.display='none'}
...
function copyCompressed(){
  ...
  for(var k in cats){
    ...
    for(var i=0;i<cards.length;i++){
      var cc2=cards[i];
      text+='- '+(cc2.name||cc2.event||cc2.rule||('#'+(i+1)));
...
function buildLoreContext(lore){
  ...
  if(lore.characters&&lore.characters.length){
    ...
    for(var i=0;i<lore.characters.length;i++){
      var cc3=lore.characters[i];
      parts.push('- '+cc3.name+(cc3.role?' ('+cc3.role+')':'')+(cc3.personality?': '+cc3.personality:''));
    }
  }
  if(lore.settings&&lore.settings.length){
    parts.push('## 场景');
    for(var i=0;i<lore.settings.length;i++){
      var ss=lore.settings[i];
      parts.push('- '+ss.name+(ss.type?' ['+ss.type+']':'')+(ss.description?': '+ss.description.substring(0,80):''));
    }
  }
  if(lore.plotPoints&&lore.plotPoints.length){
    parts.push('## 情节节点');
    for(var i=0;i<lore.plotPoints.length;i++){
      var pp=lore.plotPoints[i];
      parts.push('- '+pp.event+(pp.status?' ['+pp.status+']':''));
    }
  }
  if(lore.worldRules&&lore.worldRules.length){
    parts.push('## 世界观规则');
    for(var i=0;i<lore.worldRules.length;i++){
      var ww=lore.worldRules[i];
      parts.push('- '+ww.rule+(ww.category?' ['+ww.category+']':''));
    }
  }
  return parts.join('\\n');
}
...
async function sendMsg(){`;
  ...
  c = c.substring(0, hlprStart) + newH + c.substring(hlprEnd);
  patches++;
  console.log("OK: helpers replaced");
} else console.log("FAIL: helpers", hlprStart, hlprEnd);

// ============================================================
// 3. Replace compress modal HTML
// ============================================================
const modStart = c.indexOf('<div class="modal-overlay" id="compressModal">');
const modEnd = c.indexOf('</div>\r\n\r\n<script>', modStart);
if (modStart > 0 && modEnd > 0) {
  ...
  c = c.substring(0, modStart) + newModal + c.substring(modEnd);
  patches++;
  console.log("OK: modal replaced");
} else console.log("FAIL: modal", modStart, modEnd);

// ============================================================
// Rest written separately due to complexity
// ============================================================
console.log("\nPatches applied:", patches);
c = c; // prevent unused warning

// Write intermediate state
fs.writeFileSync("C:/Users/zhong/Documents/ChatStory/index.html", c, "utf8");
console.log("Intermediate written");