...
// Re-apply patches 1-3 that we know are clean
let orig = c;
...
// Now apply JS patches one at a time and check braces
function checkBraces(label) {
  let d = 0;
  for (let i = 0; i < orig.length; i++) { if (orig[i] === '{') d++; if (orig[i] === '}') d--; }
  console.log(label + ": braces =", d);
  return d;
}
checkBraces("Base");

// Patch 4: JS functions
const sm = 'async function sendMsg(){';
const jsFns = `async function compressContext(){
  ...
  try{
    ...
    var jsonStr=raw;
    var mm=raw.match(/\x60\x60\x60(?:json)?\\s*\\n?([\\s\\S]*?)\\n?\x60\x60\x60/);
    if(mm)jsonStr=mm[1];
    mm=jsonStr.match(/(\\{[\\s\\S]*\\})/);
    if(mm)jsonStr=mm[1];
    var lore;
    ...
    updateProgress(100,'❌ '+e.message);G('pubFill').style.background='#e05555';
    await new Promise(function(r){setTimeout(r,2500)});hideProgress(500);toast('❌ '+e.message,'error');
  }
...
orig = orig.split(sm).join(jsFns);
checkBraces("After JS functions");

// Patch 5: State init
orig = orig.split('st.convs=st.convs||[]; st.settings=st.settings||{}; st.version=st.version||0; st.rvEntries=st.rvEntries||[];').join('st.convs=st.convs||[]; st.settings=st.settings||{}; st.version=st.version||0; st.lorebook=st.lorebook||{characters:[],settings:[],plotPoints:[],worldRules:[]}; st.rvEntries=st.rvEntries||[];');
checkBraces("After state init");

// Patch 6: System prompt injection
...
orig = orig.split(oldSys).join(newSys);
checkBraces("After sys prompt");

// Patch 7: Sidebar section
const secMemEnd = '<div class="side-sec" id="secBackup"';
const loreSec = '<div class="side-sec" id="secLore"><div class="side-sec-hdr collapsed" onclick="toggleSec(\'secLore\')"><span>📖 世界观 <span class="arr">▼</span></span></div><div class="side-sec-body hidden"><div class="pad" id="loreSideList"><div style="padding:8px;font-size:11px;color:var(--text-secondary);text-align:center">点击 🗜️ 提取世界观</div></div></div></div>' + secMemEnd;
orig = orig.split(secMemEnd).join(loreSec);
checkBraces("After sidebar");

// Patch 8: renderAll hook
orig = orig.split('renderMems();renderSidebar();').join('renderLoreSidebar();renderMems();renderSidebar();');
checkBraces("After renderAll");

// Final check
const ss = orig.indexOf("<script>");
const se = orig.indexOf("</script>", ss);
try { new Function(orig.substring(ss+8, se)); console.log("JS: OK"); }
