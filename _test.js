...
function showProgress(title){
  ...
  if(!el){
    ...
    el.innerHTML='<div class="pub-progress-text" id="pubTitle"></div><div class="pub-progress-bar"><div class="pub-progress-fill" id="pubFill"></div></div><div class="pub-progress-step" id="pubStep"></div><div class="pub-progress-time" id="pubElapsed" style="font-size:10px;color:var(--text-secondary);margin-top:2px"></div><div class="pub-progress-detail" id="pubDetail" style="display:none;max-height:200px;overflow-y:auto;text-align:left;font-size:10px;color:var(--text-secondary);background:var(--hover);border-radius:8px;padding:8px 12px;margin-top:8px;font-family:monospace;line-height:1.5"></div><button id="pubCancelBtn" onclick="cancelProgress()" style="margin-top:12px;padding:6px 20px;background:#555;color:#ccc;border:none;border-radius:6px;cursor:pointer;font-size:12px">✕ 取消</button>';
  ...
  var cb=G('pubCancelBtn');if(cb)cb.style.display='';
  el.style.display='';_progressStart=Date.now();if(_progressTimer)clearInterval(_progressTimer);_progressTimer=setInterval(function(){var e=G('pubElapsed');if(e){var s=Math.floor((Date.now()-_progressStart)/1000);e.textContent='⏱ '+s+'s'}},1000);
}
...
async function cloudImport(){
  ...
  try{
    ...
    renderGVs();updateGvBadge();renderGvReasons();renderLoreSidebar();renderMems();renderSidebar();
  }catch(e){
    updateProgress(100,'❌ 失败: '+e.message);
    G('pubFill').style.background='#e05555';
    sts.textContent='❌ 失败: '+e.message;
...
async function regenerateResponse(ai){
  ...
  if(lorebook){var loreText=buildLoreContext(lorebook);if(loreText)sys=(sys?sys+'\n\n':'')+'# 小说世界观 (Lorebook)\n'+loreText;}
...
async function compressContext(){
  ...
  var dl=G("pubDetail");if(dl){dl.style.display="";dl.innerHTML="";}else{var p=G("pubProgress");if(p){dl=document.createElement("div");dl.id="pubDetail";dl.className="pub-progress-detail";dl.style.cssText="max-height:200px;overflow-y:auto;text-align:left;font-size:10px;color:var(--text-secondary);background:var(--hover);border-radius:8px;padding:8px 12px;margin-top:8px;font-family:monospace;line-height:1.5";p.insertBefore(dl,p.lastElementChild)}}
  ...
  updateProgress(70,"🔍 扫描JSON结构 ("+raw.length.toLocaleString()+"字符)...");
  await new Promise(function(r){setTimeout(r,150)});
  var jsonStr=raw;
  var BT="`";
  var btr=raw.match(new RegExp(BT+BT+BT+"(?:json)?\\s*\\n?([\\s\\S]*?)\\n?"+BT+BT+BT));
  if(btr){jsonStr=btr[1];addLog("🔍 检测到Markdown代码块 => "+jsonStr.length.toLocaleString()+"字符");updateProgress(72,"🔍 剥离Markdown包装...")}
  await new Promise(function(r){setTimeout(r,100)});
  btr=jsonStr.match(/(\{[\s\S]*\})/);
  if(btr){jsonStr=btr[1];addLog("🔍 JSON对象已定位 => "+jsonStr.length.toLocaleString()+"字符");updateProgress(74,"🔍 定位JSON对象...")}
  await new Promise(function(r){setTimeout(r,100)});
  var lore;
  try{lore=JSON.parse(jsonStr);addLog("✅ JSON解析成功 ("+jsonStr.length.toLocaleString()+"字符)");updateProgress(77,"✅ JSON解析成功")}catch(e){
    addLog("⚠️ JSON解析失败: "+e.message.substring(0,80));
    updateProgress(73,"⚠️ 修复JSON格式...");
    await new Promise(function(r){setTimeout(r,100)});
    jsonStr=jsonStr.replace(/,\s*}/g,"}").replace(/,\s*\]/g,"]");
    try{lore=JSON.parse(jsonStr);addLog("✅ JSON修复成功");updateProgress(77,"✅ JSON修复成功")}catch(e2){throw new Error("JSON解析失败: "+e2.message)}
  }
  await new Promise(function(r){setTimeout(r,100)});
...
var _loreData=null;var _loreStats="";var _loreTab="characters";

function renderLoreModal(lore,stats){
  G("compressStats").textContent=stats;_loreTab="characters";
...
function saveLorebook(){
  ...
  var cats=["characters","settings","plotPoints","worldRules"];var added=0,updated=0;
  for(var ci=0;ci<cats.length;ci++){
    ...
    for(var ni=0;ni<newCards.length;ni++){
      ...
      if(existing){for(var k in nc){if(nc[k])existing[k]=nc[k]}updated++}
...
function copyCompressed(){
  ...
  for(var k in cats){var cards=_loreData[k]||[];if(!cards.length)continue;
    text+="## "+cats[k]+"\n";
    for(var i=0;i<cards.length;i++){var c2=cards[i];text+="- "+(c2.name||c2.event||c2.rule||("#"+(i+1)));
      for(var f in c2){if(f!=="name"&&f!=="event"&&f!=="rule"&&c2[f])text+=" | "+f+": "+c2[f]}text+="\n";}
    text+="\n";
  }
...
function buildLoreContext(lore){
  ...
  if(lore.characters&&lore.characters.length){parts.push("## 角色");
    for(var i=0;i<lore.characters.length;i++){var c3=lore.characters[i];parts.push("- "+c3.name+(c3.role?" ("+c3.role+")":"")+(c3.personality?": "+c3.personality:""));}}
  if(lore.settings&&lore.settings.length){parts.push("## 场景");
    for(var i=0;i<lore.settings.length;i++){var s2=lore.settings[i];parts.push("- "+s2.name+(s2.type?" ["+s2.type+"]":"")+(s2.description?": "+s2.description.substring(0,80):""));}}
  if(lore.plotPoints&&lore.plotPoints.length){parts.push("## 情节节点");
    for(var i=0;i<lore.plotPoints.length;i++){var p2=lore.plotPoints[i];parts.push("- "+p2.event+(p2.status?" ["+p2.status+"]":""));}}
  if(lore.worldRules&&lore.worldRules.length){parts.push("## 世界观规则");
    for(var i=0;i<lore.worldRules.length;i++){var w2=lore.worldRules[i];parts.push("- "+w2.rule+(w2.category?" ["+w2.category+"]":""));}}
  return parts.join("\n");
}

function delLoreCard(cat,idx){
  if(!confirm("删除这张卡片？"))return;
  st.lorebook[cat].splice(idx,1);save();renderLoreSidebar();toast("已删除","info");
}
