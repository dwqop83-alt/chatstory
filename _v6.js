...
function post(path,ck,data){return new Promise(r=>{const b=JSON.stringify(data);const req=http.request({hostname:"localhost",port:8080,path,method:"POST",headers:{Cookie:ck||"","Content-Type":"application/json","Content-Length":b.length}},res=>{let t="";res.on("data",c=>t+=c);res.on("end",()=>{try{r(JSON.parse(t))}catch(e){r({error:t})}})});req.write(b);req.end()})}
async function main(){
  const ck=await login();
  console.log("=== Test Gitee Push ===");
  const t0=Date.now();
  const r=await post("/api/git/push",ck,{message:"Test gitee push "+new Date().toISOString(),token:"b6df2c768b72835f8fad74d052509656",repo:"middle000/story_-project",branch:"main"});
